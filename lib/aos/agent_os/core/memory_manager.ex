defmodule AOS.AgentOS.Core.MemoryManager do
  ...
  @cleanup_interval 24 * 60 * 60 * 1000 # Once a day
  ...
  defp delete_failed_executions do
    ...
    query = from e in "agent_executions", 
  ...
  defp cap_domain_successes do
    ...
    Enum.each(domains, fn domain ->
      ...
      count_query = from e in "agent_executions", 
      ...
      if count > cap do
        ...
        Logger.info("[MemoryManager] Domain '#{domain_str}' has #{count} records. Capping to #{cap} (Deleting #{over_count} oldest).")
        
        # Delete the oldest N records
        # Subquery to find IDs of oldest N records
        ids_to_delete_query = from e in "agent_executions",
                              where: e.domain == ^domain_str and e.success == true,
        ...
        Logger.info("[MemoryManager] Successfully removed #{deleted} overflow records for '#{domain_str}'.")
      end
    end)
  end
