defmodule AOS.AgentOS.Roles.LLM do
      ...
      case execute_call_raw(nil, current_history, Keyword.put(opts, :tools, mcp_tools)) do
        {:ok, %{"tool_calls" => tool_calls} = meta} when not is_nil(tool_calls) ->
          ...
          tool_results = Enum.map(tool_calls, fn tc ->
            res = execute_single_tool(tc, notify_pid)
            {tc["id"], tc["name"], res}
          end)

          assistant_message = {"assistant", %{tool_calls: tool_calls}}
          tool_result_messages = Enum.map(tool_results, fn {id, name, res} -> 
        ...
        {:ok, %{"text" => text} = meta} -> {:ok, Map.merge(merge_meta(acc_meta, meta), %{text: text})}
        {:error, reason} -> {:error, reason}
      end
    end
  end

  defp execute_single_tool(%{"name" => full_name, "arguments" => args}, notify_pid) do
    parts = String.split(full_name, "__", parts: 2)
    {server_id, tool_name} = case parts do
      [s, t] -> {s, t}
      [t] -> {"internal", t}
    end
    ...
    result =
      case request_tool_confirmation(server_id, tool_name, args, notify_pid) do
        :approved ->
    ...
    if notify_pid, do: send(notify_pid, {:workflow_step_completed, display_name, %{result: result}})
    result
  end

  defp request_tool_confirmation(server_id, tool_name, args, notify_pid) do
    if dangerous_tool?(server_id, tool_name) and notify_pid do
  ...
  defp dangerous_tool?("internal", tool_name) when tool_name in ["execute_command", "write_file", "replace"], do: true
  defp dangerous_tool?(_, _), do: false
  ...
  defp format_prompt_with_history(prompt, history) do
    ...
    history_str = Enum.map_join(history, "\n", fn {role, content} -> 
  ...
  defp execute_call_api(prompt, history, opts) do
    ...
    case HTTPoison.post(url, body, headers, [timeout: 60_000, recv_timeout: 60_000]) do
      ...
      {:ok, %HTTPoison.Response{status_code: status, body: _body}} when status in [429, 500, 502, 503, 504] ->
  ...
  defp handle_retry_raw(prompt, history, opts, current_model, retry_count) do
    if retry_count < @max_retries do
      new_model = if Application.get_env(:aos, :cliproxy_api), do: select_fallback_model(current_model), else: current_model
      wait_time = @base_sleep_ms * :math.pow(2, retry_count) |> round()
      
      Logger.info("Retrying (#{retry_count + 1}/#{@max_retries}) with model #{new_model} in #{wait_time}ms...")
      Process.sleep(wait_time)
      
      new_opts = opts |> Keyword.put(:retry_count, retry_count + 1) |> Keyword.put(:model, new_model)
  ...
  def list_models do
    ...
    case HTTPoison.get(url, headers) do
      ...
      _ -> {:error, :failed_to_list_models}
    end
  end

  defp prepare_request(base_url, model, prompt, history, tools) do
    messages = Enum.flat_map(history, fn 
      {"user", content} -> [%{role: "user", content: scrub_utf8(content)}]
      {"assistant", %{tool_calls: calls}} -> 
        [%{role: "assistant", tool_calls: Enum.map(calls, fn c -> %{id: c["id"], type: "function", function: %{name: c["name"], arguments: Jason.encode!(c["arguments"])}} end)}]
      {"assistant", content} -> [%{role: "assistant", content: scrub_utf8(content)}]
    ...
    messages = if prompt, do: messages ++ [%{role: "user", content: scrub_utf8(prompt)}], else: messages

    payload = %{
      model: String.replace(model, ~r|^models/|, ""),
      messages: messages,
      tools: if(tools, do: Enum.map(tools, fn t -> %{type: "function", function: %{name: "#{t["server_id"]}__#{t["name"]}", description: t["description"], parameters: t["inputSchema"]}} end))
    }
  ...
  defp parse_raw_response(_model, body) do
    ...
    cond do
      choice && choice["tool_calls"] ->
        calls = Enum.map(choice["tool_calls"], fn tc -> %{"id" => tc["id"], "name" => tc["function"]["name"], "arguments" => Jason.decode!(tc["function"]["arguments"] || "{}")} end)
        {:ok, build_tool_call_response(calls, usage, model)}
      choice && choice["content"] -> {:ok, build_text_response(choice["content"], usage, model)}
  ...
  def estimate_usage(prompt, history, result_text) do
    ...
    result_chars = if is_binary(result_text), do: String.length(result_text), else: String.length(inspect(result_text))
  ...
  defp pricing_for_model(model) do
    ...
    case matched do
      ...
      {_key, value} when is_map(value) ->
        %{
          input_per_1k: Map.get(value, :input_per_1k, Map.get(value, "input_per_1k", @default_input_cost_per_1k)),
          output_per_1k: Map.get(value, :output_per_1k, Map.get(value, "output_per_1k", @default_output_cost_per_1k))
        }
  ...
  defp merge_meta(acc, meta) do
    ...
    %{
      ...
      "cost_usd" => Float.round((acc["cost_usd"] || acc[:cost_usd] || 0.0) + (meta["cost_usd"] || meta[:cost_usd] || 0.0), 6),
